...
async function sign$1(message, privateKey) {
    const [signature, recoveryId] = await sign$2(message, privateKey, {
        recovered: true,
        der: false
    });
...
class Multiaddr {
    ...
    decapsulate(addr) {
        ...
        if (i < 0) {
            throw new InvalidParametersError$1(`Address ${this.toString()} does not contain subaddress: ${addr.toString()}`);
        }
...
class ENR extends RawEnr {
    ...
    static create(kvs = {}, seq = BigInt(1), signature) {
        ...
        try {
            ...
            log$1.error("Could not calculate peer id for ENR", e);
        }
        return enr;
    }
    get nodeId() {
    ...
    get shardInfo() {
        if (this.rs && this.rsv) {
            log$1.warn("ENR contains both `rs` and `rsv` fields.");
        }
...
const log = new Logger$1("enr:decoder");
class EnrDecoder {
...
function checkSignature(seq, kvs, enr, signature) {
    const rlpEncodedBytes = hexToBytes$2(encode([seq, ...kvs]));
